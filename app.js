...
const state = {view:'diary', id:null, q:'', year:'all', type:'all', onlyCovers:false, sort:'dateDesc', drawerPerson:null, toast:'', editPerf:null, history:[], future:[]};
...
function navigate(view,id=null, push=true){ if(push){ state.history.push(snapshot()); state.future=[]; } state.view=view; state.id=id===null?null:Number(id); state.drawerPerson=null; state.editPerf=null; window.scrollTo({top:0,behavior:'smooth'}); render(); }
...
function hero(){ return `<header class="hero"><div class="heroInner"><div><div class="eyebrow">Personal Theatre Archive · Web App V1</div><h1>Theatre Attendance Diary</h1><div class="sub">A cross-reference diary built from <b>${esc(DB.meta.sourceFile)}</b>. Numeric IDs power the people, shows, roles, venues, performances, appearances, and creative credits underneath.</div></div><div class="heroTools"><button class="btn" data-action="go-back" ${state.history.length?'':'disabled'}>← Back</button><button class="btn" data-action="go-forward" ${state.future.length?'':'disabled'}>Forward →</button><button class="btn" data-action="random">Random show 🎲</button></div></div><nav class="nav">${navButton('diary','Diary')}${navButton('people','People')}${navButton('roleVariety','Role Counts')}${navButton('shows','Shows')}${navButton('venues','Venues')}${navButton('covers','Covers')}${navButton('edits','Edits')}</nav></header>`; }
function appShell(content, drawer=true){ return `${hero()}<main class="main">${content}</main>${drawer && state.drawerPerson ? renderPersonDrawer(state.drawerPerson) : ''}${state.toast?`<div class="toast">${esc(state.toast)}</div>`:''}`; }
...
function personCard(pid){ const p=people[pid], st=personStats[pid]||{}, variety=roleVarietyByPerson[pid]; return `<article class="card click" data-action="open-person-page" data-id="${pid}"><div class="showTitle" style="font-size:23px">${esc(p.name)}</div><div class="meta">${esc((p.kinds ? p.kinds.join(' · ') : '') || '')}</div><div class="badgeRow"><span class="badge">${st.performances||0} perfs</span><span class="badge">${st.shows||0} shows</span><span class="badge">${(variety && variety.count)||0} role count</span>${st.creativeCredits?`<span class="badge">${st.creativeCredits} creative</span>`:''}</div></article>`; }
...
function renderRole(rid){ const r=roles[rid]; if(!r) return appShell('<div class="empty">Role not found.</div>'); const norm=normalizeRoleName(r.name), siblingIds=Object.values(roles).filter(x=>x.showId===r.showId && normalizeRoleName(x.name)===norm).map(x=>x.id); const apps=siblingIds.flatMap(id=>byRole[id]||[]).sort(sortAppsDesc); const byP={}; for(const a of apps){ if(!byP[a.personId]) byP[a.personId]={count:0,covers:0,perfs:new Set()}; const item=byP[a.personId]; item.count++; item.covers+=a.coverType?1:0; item.perfs.add(a.performanceId); } const rows=Object.entries(byP).sort((a,b)=>b[1].perfs.size-a[1].perfs.size || personName(a[0]).localeCompare(personName(b[0]))).map(([pid,x])=>`<tr><td>${personBtn(pid)}</td><td>${x.perfs.size}</td><td>${x.covers||''}</td><td>${[...x.perfs].slice(0,4).map(id=>`<button class="linkBtn" data-action="open-performance" data-id="${id}">${fmtDate(performances[id].date)}</button>`).join(', ')}</td></tr>`).join(''); return appShell(`${breadcrumb('Role')}<section class="detailHero"><div class="smallCaps">Role history</div><div class="detailTitle">${esc(r.name)}</div><div class="detailMeta"><span>${showBtn(r.showId)}</span><span>${apps.length} appearances</span><span>${Object.keys(byP).length} people</span><span>normalized with ${siblingIds.length} role variant${siblingIds.length===1?'':'s'}</span></div></section><div class="twoCol"><section class="panel"><div class="sectionHeader"><h2>Appearances</h2></div><div class="miniRows">${apps.map(miniAppearance).join('')}</div></section><aside class="panel"><div class="sectionHeader"><h2>People Seen in Role</h2></div><div class="tableWrap"><table><thead><tr><th>Person</th><th>Perfs</th><th>Covers</th><th>Dates</th></tr></thead><tbody>${rows}</tbody></table></div></aside></div>`); }
...
function venueCard(vid){ const v=venues[vid], st=venueStats[vid]||{}; return `<article class="card click" data-action="open-venue" data-id="${vid}"><div class="showTitle" style="font-size:23px">${esc(v.name)}</div><div class="meta">${esc(v.city||'')}</div><div class="badgeRow"><span class="badge">${st.performances} perfs</span><span class="badge">${st.shows} shows</span>${st.first?`<span class="badge">${fmtDate(st.first)} → ${fmtDate(st.recent)}</span>`:''}</div></article>`; }
...
function renderCovers(){ const rows=appearances.filter(a=>a.coverType).slice().sort(sortAppsDesc); return appShell(`${renderStats()}<div class="sectionHeader"><h2>Covers / Special Appearances</h2><span class="muted">${rows.length.toLocaleString()}</span></div><div class="grid">${rows.map(a=>`<article class="card click" data-action="open-performance" data-id="${a.performanceId}"><div class="smallCaps">${esc(a.coverLabel||'cover')}</div><div class="showTitle" style="font-size:22px">${esc(personName(a.personId))}</div><div class="meta">${esc(roleName(a.roleId))} · ${esc(perfTitle(a.performanceId))}<br>${fmtDate((performances[a.performanceId] && performances[a.performanceId].date))} · ${esc(venueLabel(a.performanceId))}</div>${badge(a.coverLabel,a.coverType)}</article>`).join('')}</div>`); }
...
app.addEventListener('click', e=>{
  const btn=e.target.closest('[data-action]'); if(!btn) return; const action=btn.dataset.action, id=btn.dataset.id ? Number(btn.dataset.id) : null;
